...
class _copingCalendarState extends State<copingCalendar> {
  ...
  void deleteEvent(Meeting event) {
    MyList.meetings.remove(event);
    MyList.saveMeetings();
    setState(() {
      // Refresh the calendar to reflect the changes
      events = MeetingDataSource(MyList.meetings);
    });
  }

  void addEventToCalendar(Meeting newEvent) {
    MyList.meetings.add(newEvent);
    MyList.saveMeetings();
    ...
    if (fetchEvents) {
      return (Scaffold(
        appBar: AppBar(
          ...
          title: const Text('Plan Your Wellbeing!'),
          actions: [
            Row(
              children: [
                IconButton(
                  onPressed: () {
                    showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AddCustomEventDialog(addEventToCalendar);
                      },
                    );
                  },
              ...
              mainAxisAlignment:
                  MainAxisAlignment.spaceBetween, // Separates the icons
            ),
          ],
        ),
        body: Column(
...
class AddCustomEventDialog extends StatefulWidget {
  final void Function(Meeting newEvent)
      addEventToCalendar; // Define the callback

  AddCustomEventDialog(
      this.addEventToCalendar); // Constructor that takes the callback

  @override
  _AddCustomEventDialogState createState() => _AddCustomEventDialogState();
}

class _AddCustomEventDialogState extends State<AddCustomEventDialog> {
  DateTime selectedDate = DateTime.now();
  String selectedActivity = 'Journal';
  String description = '';

  @override
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      // Wrap with SingleChildScrollView to prevent layout errors
      child: AlertDialog(
          ...
          TextButton(
            ...
            onPressed: () async {
              final meeting = Meeting(
                ...
                isAllDay: true, // Set the event as an all-day event
                description: description,
              );

              widget.addEventToCalendar(meeting); // Call the callback function
